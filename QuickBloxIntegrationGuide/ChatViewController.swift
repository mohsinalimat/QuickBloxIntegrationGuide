...
class ChatViewController: QMChatViewController, QMChatServiceDelegate, UIActionSheetDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, QMChatAttachmentServiceDelegate, QMChatConnectionDelegate, QMChatCellDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let currentUser = QMServicesManager.instance().currentUser() {
            ...
            self.inputToolbar?.contentView?.textView?.placeHolder = "SA_STR_MESSAGE_PLACEHOLDER"
            ...
            if self.dialog.type == QBChatDialogType.Private {
                ...
                self.dialog.onUserIsTyping = {
                    ...
                    if QMServicesManager.instance().currentUser()?.ID == userID {
                        return
                    }
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        //QMServicesManager.instance().currentDialogID = self.dialog.ID!
    ...
    override func viewDidDisappear(animated: Bool) {
        ...
        //QMServicesManager.instance().currentDialogID = ""
...
//        }
    ...
    func updateTitle() {
        ...
        if self.dialog.type != QBChatDialogType.Private {
            ...
            if let recipient = QMServicesManager.instance().usersService.usersMemoryStorage.userWithID(UInt(self.dialog!.recipientID)) {
    ...
    func loadMessages() {
        ...
        QMServicesManager.instance().chatService.messagesWithChatDialogID(currentDialogID, completion: {
    ...
    func sendReadStatusForMessage(message: QBChatMessage) {
        ...
        if (message.readIDs == nil || !message.readIDs!.contains(currentUserID)) {
            QMServicesManager.instance().chatService.readMessage(message, completion: { (error: NSError?) -> Void in
    ...
    override func didPickAttachmentImage(image: UIImage!) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), {
            ...
            dispatch_async(dispatch_get_main_queue(), {
                ...
                QMServicesManager.instance().chatService.sendAttachmentMessage(message, toDialog: self!.dialog, withAttachmentImage: resizedImage, completion: {
    ...
    func sendMessage(message: QBChatMessage) {
        ...
        // Sending message.
        QMServicesManager.instance().chatService.sendMessage(message, toDialogID: self.dialog.ID!, saveToHistory: true, saveToStorage: true) { (error: NSError?) -> Void in
            
            if error != nil {
                
                //QMMessageNotificationManager.showNotificationWithTitle("SA_STR_ERROR", subtitle: error?.localizedDescription, type: QMMessageNotificationType.Warning)
            }
        }
        ...
        if message.readIDs != nil {
            ...
            if !messageReadIDs.isEmpty {
                for readID in messageReadIDs {
                    let user = QMServicesManager.instance().usersService.usersMemoryStorage.userWithID(UInt(readID))
        ...
        if message.deliveredIDs != nil {
            ...
            if !messageDeliveredIDs.isEmpty {
                for deliveredID in messageDeliveredIDs {
                    let user = QMServicesManager.instance().usersService.usersMemoryStorage.userWithID(UInt(deliveredID))
                ...
                if deliveredLogins.count > 0 {
                    statusString += "SA_STR_DELIVERED_STATUS" + ": " + deliveredLogins.joinWithSeparator(", ")
                }
            }
        }
    ...
    override func topLabelAttributedStringForItem(messageItem: QBChatMessage!) -> NSAttributedString? {
        ...
        if let topLabelText = QMServicesManager.instance().usersService.usersMemoryStorage.userWithID(messageItem.senderID)?.login {
    ...
    override func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        ...
        if (indexPath.section == lastSection && indexPath.item == (self.collectionView?.numberOfItemsInSection(lastSection))! - 1) {
            ...
            QMServicesManager.instance().chatService.loadEarlierMessagesWithChatDialogID(dialogID).continueWithBlock({
    ...
    func refreshAndReadMessages() {
        ...
        //SVProgressHUD.showWithStatus("SA_STR_LOADING_MESSAGES", maskType: SVProgressHUDMaskType.Clear)
        self.loadMessages()
